...
class UserIntent(BaseModel):
    include_node_types: List[str] = Field(..., description="Types of nodes to include (e.g., Room, Wall)")
    filter_node_attributes: Dict[str, str] = Field(default_factory=dict, description="Filters on node attributes like level, usage")
    relations: List[str] = Field(..., description="Type of relationship to use (e.g., connected_by, adjacent_to)")
...
def parse_user_intent(state):
    ...
    print(f"[parse_user_intent] Available node types: {schema['node_types']}")
    ...
    try:
        ...
        print(f"   - include_node_types: {generated_query['include_node_types']}")
